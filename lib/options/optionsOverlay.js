...
/** @lends OptionsOverlay */
{
  ...
  onReady: function(options) {
    ...
    gDevTools.on("pref-changed", function(eventType, data) {
      Trace.sysout("firebug.updateTheme; ", data);
      if(data.pref === 'devtools.theme')
      {
        let debuggerStylesUrl = self.data.url("firebug-theme/options.css");
        loadSheet(options.panel.panelWin, debuggerStylesUrl, "author");
        loadSheet(options.panel.panelWin,
